...
def write_name(ai_settings, stats, table_of_elements, event, input):
# Faz parte de events
...
def events(ai_settings, screen, ship, shots, aliens, alien_bombs, stats, table_of_elements, shots_aliens, play_button):
# Responde as ações do usuário
    for event in pygame.event.get():
        if event.type == pygame.QUIT: 
            ...
            write_name(ai_settings, stats, table_of_elements, event, stats.name)
        elif event.type == pygame.KEYUP:
            keyup_events(event, ai_settings, ship)
        elif event.type == pygame.MOUSEBUTTONDOWN:
            mouse_x, mouse_y = pygame.mouse.get_pos()
            check_play_button(ai_settings, screen, ship, shots, aliens, alien_bombs, stats, play_button, table_of_elements, shots_aliens, mouse_x, mouse_y)


def check_play_button(ai_settings, screen, ship, shots, aliens, alien_bombs, stats, play_button, table_of_elements, shots_aliens, mouse_x, mouse_y):
# Faz parte de checar_eventos
    ...
    if button_clicked and not stats.game_active and ai_settings.aliens == False and ai_settings.bombs == False and ai_settings.questions == False and ai_settings.writen_name == True and stats.name != 'digite seu nome...':
        ...
        table_of_elements.prep_score()
        table_of_elements.prep_high_score()
        table_of_elements.prep_level()
        table_of_elements.prep_ships()
        table_of_elements.prep_name()
        table_of_elements.prep_timer()
        table_of_elements.prep_question()
        table_of_elements.prep_answers()
        # Tira aliens e ship
        aliens.empty()
        alien_bombs.empty()
        shots.empty()
...
def drawn_shots(ai_settings, screen, ship, shots, stats, shots_aliens):
    ...
    for shot in shots.sprites(): 
        ...
        if ai_settings.shoting == True and len(shots) < ai_settings.bullets_allowed: 
        # if ai_settings.shoting == True and len(shots): # colocar esse if e tirar o de cima == bug legal 
            shots.add(new_shot)
            stats.bullet_shots += 1
    for shot_alien in shots_aliens.sprites(): 
        shot_alien.drawn_shot() 


def screen_update(ai_settings, screen, ship, shots, aliens, alien_bombs, stats, play_button, table_of_elements, shots_aliens, mother_ship):
# Atualiza as informações na tela
    ...
    table_of_elements.show_elements() # Desenha a tabela de pontos
    # Preparar timer
    ...
    if ai_settings.death_ray == True and stats.ships_left == 0:
        mother_ship.image = pygame.image.load('imagens/nave_mãe/Nave mãe-2.png.png')
        mother_ship.image = pygame.transform.scale(mother_ship.image, (64, 64))
        mother_ship.show() # Desenha a nave mãe
...
def check_high_score(stats, table_of_elements):
# Faz parte de check_alien_shot_collision
    if stats.score > stats.high_score: 
        # Registrar nome do primeiro
...
def shot_collision(shots, stats, table_of_elements, enemy, enemy_points):
# Faz parte de check_alien_shot_collision
    ...
    if colisoes:
        for enemy in colisoes.values():
            ...
            table_of_elements.prep_score()
            check_high_score(stats, table_of_elements)
        if stats.score <= stats.high_score: 
            # Som de pontuação
...
def check_alien_shot_collision(ai_settings, screen, shots, aliens, alien_bombs, ship, stats, table_of_elements, shots_aliens, mother_ship):
# Remove os aliens que sofreram colisão # Faz parte de shots_update
    shot_collision(shots, stats, table_of_elements, aliens, ai_settings.alien_points)
    shot_collision(shots, stats, table_of_elements, alien_bombs, ai_settings.bomb_points)
    if pygame.sprite.spritecollideany(ship, shots_aliens): 
        collide_ship(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship)


def shots_update(ai_settings, screen, shots, aliens, alien_bombs, ship, stats, table_of_elements, shots_aliens, mother_ship):
# Atualiza os shots e remove os shots fora da tela
    ...
    check_alien_shot_collision(ai_settings, screen, shots, aliens, alien_bombs, ship, stats, table_of_elements, shots_aliens, mother_ship)
...
def check_screen_botton(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship):
# Faz parte de aliens_update
    screen_rect = screen.get_rect()
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            collide_ship(ai_settings, screen, stats, ship, aliens, shots, table_of_elements, shots_aliens, mother_ship)
            break
...
def collide_ship(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship):
...
def check_alien_quantity(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens):
# Faz parte de aliens_update
    alien_values = total_aliens(ai_settings, stats, ship, screen, aliens)
...
def check_question(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship, question):
# Faz parte parte de collede_questions
    ...
    if stats.json_questions_table['perguntas'][stats.current_question][resposta] == stats.json_questions_table['perguntas'][stats.current_question]['resposta_correta']:
        ...
        pygame.draw.line(screen, ai_settings.alien_bullet_color, (mother_ship.rect_mother_ship.centerx, mother_ship.rect_mother_ship.centery), (ship.rect.centerx, ship.rect.centery), 5)
        mother_ship.change_image('Nave mãe-2.png.png')
        pygame.display.update()
        collide_ship(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship)


def collide_questions(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship):
# Faz parte de aliens_update
    for shot in shots:
        if pygame.Rect.colliderect(shot.rect, mother_ship.rect_mother_ship):
            shot.speed_factor *= -1
            # Som de colisão com a nave mãe
            pygame.mixer.music.load('sons/deflection.wav')
            pygame.mixer.music.set_volume(0.75)
            pygame.mixer.music.play()
        elif pygame.Rect.colliderect(shot.rect, mother_ship.qa_rect):
            check_question(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship, 'A')
            break
        elif pygame.Rect.colliderect(shot.rect, mother_ship.qb_rect):
            check_question(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship, 'B')
            break
        elif pygame.Rect.colliderect(shot.rect, mother_ship.qc_rect):
            check_question(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship, 'C')
            break
        elif pygame.Rect.colliderect(shot.rect, mother_ship.qd_rect):
            check_question(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship, 'D')
            break


def aliens_update(ai_settings, stats, screen, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship):
    check_alien_quantity(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens) 
    ...
    if ai_settings.questions:
        collide_questions(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship)
    else:
        mother_ship.change_image('Nave mãe-1.png.png')
    check_screen_botton(ai_settings, screen, stats, ship, aliens, alien_bombs, shots, table_of_elements, shots_aliens, mother_ship)
